...
class Qlearning(object):
    ...
    def learn_from_tuple(self, s, a, r, next_s):
        next_max_a = self.greedy_policy(next_s)
        self.tabular[s][a] = self.tabular[s][a] + self.lr * (r + self.gamma * self.tabular[next_s][next_max_a]
                                                             - self.tabular[s][a])


def runQlearning(config):
    # initialization
    if config.map == '4x4':
        maze_size = 4
    else:
    ...
    td_algo = Qlearning(config.lr, config.epsilon, config.gamma, maze_size ** 2, 4)
    ...
    for ep_i in range(0, config.episodes):
        ...
        while True:
            ...
            td_algo.learn_from_tuple(obs, action, reward, next_obs)
